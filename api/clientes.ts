...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (resource === 'credentials') return handleCredentials(req, res);
    if (resource === 'team') return handleTeam(req, res);
    if (resource === 'metricool') return handleMetricool(req, res);
    if (resource === 'servicios') return handleServicios(req, res);
    if (resource === 'contratos') return handleContratos(req, res);
...
async function handleClientes(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    const { id } = req.query;
    if (id) {
...
async function handleMetricool(req: VercelRequest, res: VercelResponse) {
  if (!METRICOOL_API_KEY) return res.status(500).json({ error: 'API Key de Metricool no configurada' });
  if (req.method !== 'GET') return res.status(405).json({ error: 'Método no permitido' });

  const { action, brandId } = req.query;
  const headers = { 'X-Mc-Auth': METRICOOL_API_KEY };

  try {
    if (action === 'brands') {
      const response = await fetch(`${METRICOOL_BASE_URL}/admin/simpleProfiles?userId=${METRICOOL_USER_ID}`, { headers });
      if (!response.ok) return res.status(response.status).json({ error: 'Error obteniendo marcas' });
    ...
    if (action === 'scheduled' && brandId) {
      ...
      const futureDate = new Date(now); futureDate.setMonth(futureDate.getMonth() + 3);
      const startDate = pastDate.toISOString().split('.')[0];
      const endDate = futureDate.toISOString().split('.')[0];
      const response = await fetch(`${METRICOOL_BASE_URL}/v2/scheduler/posts?userId=${METRICOOL_USER_ID}&blogId=${brandId}&start=${startDate}&end=${endDate}`, { headers });
      if (!response.ok) return res.status(response.status).json({ error: 'Error obteniendo posts programados' });
      return res.status(200).json(await response.json());
    }
    ...
    return res.status(500).json({ error: 'Error de conexión con Metricool' });
  }
}

async function handleServicios(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    const servicios = await prisma.servicioRecurrente.findMany({ where: { activo: true }, orderBy: [{ tipo: 'desc' }, { nombre: 'asc' }] });
    return res.status(200).json(servicios);
  }
  if (req.method === 'POST') {
    const data = req.body;
    const servicio = await prisma.servicioRecurrente.create({
      data: { codigo: data.codigo, nombre: data.nombre, descripcion: data.descripcion, categoria: data.categoria, tipo: data.tipo || 'INDIVIDUAL', serviciosIncluidos: data.serviciosIncluidos || null, precioBase: data.precioBase ? parseFloat(data.precioBase) : null, cuotaActivacion: data.cuotaActivacion ? parseFloat(data.cuotaActivacion) : null, activo: data.activo ?? true },
    });
...
async function handleContratos(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    ...
    if (dashboard === 'true') {
      const contratos = await prisma.contrato.findMany({
        ...
        include: { cliente: { select: { id: true, name: true } }, servicio: { select: { id: true, nombre: true, categoria: true } } },
      });
      const hoy = new Date();
      ...
      const porCategoria = contratos.reduce((acc: Record<string, { count: number; mrr: number }>, c) => {
      ...
      return res.status(200).json({ totalContratos: contratos.filter(c => c.estado === 'ACTIVO').length, contratosPausados: contratos.filter(c => c.estado === 'PAUSADO').length, mrrTotal: Math.round(mrrTotal * 100) / 100, arrTotal: Math.round(mrrTotal * 12 * 100) / 100, proximosVencer, porCategoria });
    }

    const where: Record<string, unknown> = {};
    if (clienteId) where.clienteId = clienteId;
    if (estado) where.estado = estado;
    const contratos = await prisma.contrato.findMany({ where, include: { cliente: { select: { id: true, name: true } }, servicio: { select: { id: true, nombre: true, categoria: true } } }, orderBy: { fechaInicio: 'desc' } });
  ...
  if (req.method === 'POST') {
    ...
    const contrato = await prisma.contrato.create({
      data: { clienteId: data.clienteId, servicioId: data.servicioId, nombre: data.nombre, descripcion: data.descripcion, importeMensual: parseFloat(data.importeMensual), periodicidad: data.periodicidad || 'MENSUAL', fechaInicio: new Date(data.fechaInicio), fechaFin: data.fechaFin ? new Date(data.fechaFin) : null, fechaRenovacion: data.fechaRenovacion ? new Date(data.fechaRenovacion) : null, autoRenovar: data.autoRenovar ?? true, estado: data.estado || 'ACTIVO', notas: data.notas },
      include: { cliente: { select: { id: true, name: true } }, servicio: { select: { id: true, nombre: true, categoria: true } } },
    });
    await prisma.historialContrato.create({ data: { contratoId: contrato.id, tipo: 'CREACION', descripcion: `Contrato creado: ${contrato.servicio.nombre} para ${contrato.cliente.name}`, valorNuevo: contrato.importeMensual } });
  ...
  if (req.method === 'PUT') {
    ...
    const contrato = await prisma.contrato.update({ where: { id }, data, include: { cliente: { select: { id: true, name: true } }, servicio: { select: { id: true, nombre: true, categoria: true } } } });
    if (contratoActual && data.importeMensual && contratoActual.importeMensual !== data.importeMensual) {
      await prisma.historialContrato.create({ data: { contratoId: id, tipo: 'CAMBIO_PRECIO', descripcion: 'Cambio de precio', valorAnterior: contratoActual.importeMensual, valorNuevo: data.importeMensual } });
    }
    if (contratoActual && data.estado && contratoActual.estado !== data.estado) {
      await prisma.historialContrato.create({ data: { contratoId: id, tipo: data.estado === 'PAUSADO' ? 'PAUSA' : data.estado === 'CANCELADO' ? 'CANCELACION' : 'REACTIVACION', descripcion: `Estado cambiado a ${data.estado}` } });
    }
  ...
  if (req.method === 'DELETE') {
    const { id } = req.body;
    await prisma.contrato.update({ where: { id }, data: { estado: 'CANCELADO' } });
    await prisma.historialContrato.create({ data: { contratoId: id, tipo: 'CANCELACION', descripcion: 'Contrato cancelado' } });
