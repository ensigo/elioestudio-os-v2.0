...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (entity === 'equipo-activo') {
      ...
      const resultado = jornadas.map((j: any) => {
        const entry = entriesActivos.find((e: any) => e.userId === j.usuarioId);
        const minutos = entry ? Math.floor((Date.now() - new Date(entry.startTime).getTime()) / 60000) : null;
        return {
          usuario: j.usuario,
          jornada: { estado: j.estado, horaInicio: j.horaInicio },
          tareaActiva: entry ? { title: entry.tarea?.title || 'Trabajando', proyecto: entry.tarea?.proyecto?.title || '', cliente: entry.tarea?.proyecto?.cliente?.name || '', minutos } : null
        };
      });
