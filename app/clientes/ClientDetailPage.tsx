...
export const ClientDetailPage: React.FC<ClientDetailPageProps> = ({ client, onBack, usuarios, currentUser, onClientUpdate }) => {
  ...
  const handleAddCredential = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setCredentials([...credentials, await res.json()]);
  ...
  const handleDeleteCredential = async (credId: string) => {
    ...
    try {
      ...
      if (res.ok) setCredentials(credentials.filter(c => c.id !== credId));
  ...
  const handleAddTeamMember = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setTeamMembers([...teamMembers, await res.json()]);
    ...
    <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
                              ...
                              <div><span className="text-xs text-gray-500">Contraseña</span>
                                <div className="flex items-center gap-2">
                                  ...
                                  <button onClick={() => setVisiblePasswords({...visiblePasswords, [cred.id]: !visiblePasswords[cred.id]})}>{visiblePasswords[cred.id] ? <EyeOff size={14} /> : <Eye size={14} />}</button>
                                </div>
                              </div>
                            </div>
      ...
      <Modal isOpen={showAddCredential} onClose={() => setShowAddCredential(false)} title="Nueva Credencial">
        <div className="space-y-4">
          ...
          <select value={newCredential.platform} onChange={e => setNewCredential({...newCredential, platform: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm">
            <option value="">Seleccionar...</option>
            {(PLATFORMS_BY_CATEGORY[newCredential.category] || []).map(p => <option key={p} value={p}>{p}</option>)}
            <option value="custom">-- Otro --</option>
          </select>
          {newCredential.platform === 'custom' && <input placeholder="Nombre plataforma" onChange={e => setNewCredential({...newCredential, customPlatform: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm" />}
          <input value={newCredential.url} onChange={e => setNewCredential({...newCredential, url: e.target.value})} placeholder="URL de acceso" className="w-full px-3 py-2 border rounded-lg text-sm" />
          <div className="grid grid-cols-2 gap-4">
            <input value={newCredential.username} onChange={e => setNewCredential({...newCredential, username: e.target.value})} placeholder="Usuario *" className="px-3 py-2 border rounded-lg text-sm" />
            <input value={newCredential.passwordEncrypted} onChange={e => setNewCredential({...newCredential, passwordEncrypted: e.target.value})} placeholder="Contraseña *" className="px-3 py-2 border rounded-lg text-sm font-mono" />
          </div>
          <input value={newCredential.email} onChange={e => setNewCredential({...newCredential, email: e.target.value})} placeholder="Email recuperación" className="w-full px-3 py-2 border rounded-lg text-sm" />
          <textarea value={newCredential.notes} onChange={e => setNewCredential({...newCredential, notes: e.target.value})} placeholder="Notas" rows={2} className="w-full px-3 py-2 border rounded-lg text-sm" />
      ...
      <Modal isOpen={!!editingCredential} onClose={() => setEditingCredential(null)} title="Editar Credencial">
        {editingCredential && (
          <div className="space-y-4">
            <select value={editingCredential.category} onChange={e => setEditingCredential({...editingCredential, category: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm">
              {CREDENTIAL_CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
            </select>
            <input value={editingCredential.platform} onChange={e => setEditingCredential({...editingCredential, platform: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm" />
            <input value={editingCredential.url || ''} onChange={e => setEditingCredential({...editingCredential, url: e.target.value})} placeholder="URL" className="w-full px-3 py-2 border rounded-lg text-sm" />
            <div className="grid grid-cols-2 gap-4">
              <input value={editingCredential.username} onChange={e => setEditingCredential({...editingCredential, username: e.target.value})} className="px-3 py-2 border rounded-lg text-sm" />
              <input value={editingCredential.passwordEncrypted} onChange={e => setEditingCredential({...editingCredential, passwordEncrypted: e.target.value})} className="px-3 py-2 border rounded-lg text-sm font-mono" />
            </div>
            <input value={editingCredential.email || ''} onChange={e => setEditingCredential({...editingCredential, email: e.target.value})} placeholder="Email" className="w-full px-3 py-2 border rounded-lg text-sm" />
            <textarea value={editingCredential.notes || ''} onChange={e => setEditingCredential({...editingCredential, notes: e.target.value})} rows={2} className="w-full px-3 py-2 border rounded-lg text-sm" />
